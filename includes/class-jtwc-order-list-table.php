...
class JTWC_Order_List_Table extends WP_List_Table
{
    protected $data = [];
    ...
    public function get_columns()
    {
        ...
        if ($sub_tab === "international" && ($cate === "no_pickup" || !$cate)) {
    ...
    {
        $sortable_columns = array(// 'id' => array( 'id', false ),
        );
    ...
    protected function column_title($item)
    {
        ...
        // Build schedule pickup row action.
        ...
        $actions['schedule_pickup'] = sprintf(
            '<a data-toggle="modal" data-target="#schedule-pick-up" data-id="%3$s" data-weight="%4$s" class="btn-open-modal" href="#">%2$s</a>',
            esc_url(wp_nonce_url(add_query_arg($edit_query_args, 'admin.php'), 'schedule_pickup_order_' . $item['id'])),
            _x('Schedule a pickup', 'List table row action', 'wp-list-table-example'),
            $item['id'],
            $item['weight']
        );
